...
public class SysGeneratorService {
    ...
    public Page<SysGenerator> findAll(Collection<SearchFilter> filters, Pageable pageable) {
        ...
        return sysLogDao.findList(params, pageable);
    }
    ...
    public byte[] generatorCode(String[] tableNames) {
        ...
        for(String tableName : tableNames){
            //查询表信息
            Map<String, String> table = sysLogDao.queryTable(tableName);
            logger.info("table="+ JSONObject.toJSONString(table));
            //查询列信息
            List<Map<String, String>> columns = sysLogDao.queryColumns(tableName);
